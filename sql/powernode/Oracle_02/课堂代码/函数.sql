...
-- 所有的组函数都是忽略空值的
SELECT   SUM(COMM) "佣金总和"   ,AVG(COMM) "平均佣金"  ,   SUM(COMM)/COUNT(COMM) "平均佣金一"
FROM EMP E;
...
SELECT   SUM(COMM)/COUNT(*) "平均佣金"  ,AVG( NVL(COMM,0) )  "平均佣金1"
FROM EMP E;
...
SELECT  DEPTNO,   MAX(SAL) "最高工资"   ,MIN(SAL) "最低工资" ,AVG(SAL) "平均工资"  ,SUM(SAL) "工资总和"
FROM EMP 
GROUP BY DEPTNO ;


SELECT  DEPTNO, JOB,  MAX(SAL) "最高工资"   ,MIN(SAL) "最低工资" ,AVG(SAL) "平均工资"  ,SUM(SAL) "工资总和"
FROM EMP 
GROUP BY DEPTNO ,JOB ;


SELECT   JOB,  MAX(SAL) "最高工资"   ,MIN(SAL) "最低工资" ,AVG(SAL) "平均工资"  ,SUM(SAL) "工资总和"
FROM EMP 
GROUP BY JOB 
ORDER BY "最高工资";
