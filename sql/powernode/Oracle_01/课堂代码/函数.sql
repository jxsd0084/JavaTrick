-- 单行函数
SELECT ENAME, LOWER( ENAME ), Length( ENAME ) AS "姓名长度"
FROM EMP;
...
SELECT  ENAME, SUBSTR( ENAME, 0 , 2 ) AS "前两个字符", RPAD( sal, 10, '*' )
FROM EMP;
...
SELECT ENAME, ( SYSDATE - HIREDATE )/365,  ROUND( ( SYSDATE - HIREDATE )/365, 0 ) AS "工作年限" FROM EMP;


SELECT TRUNC( 45.946, 2 ) AS "小数点后两位",
       TRUNC( 45.946, 0 ) AS "个位",
       TRUNC( 45.946, -1 ) AS "十位"
FROM SYS.DUAL;
...
SELECT ENAME, HIREDATE, ( HIREDATE + 90 ) AS "转正日期", add_months( HIREDATE,3 ) AS "精确转正日期"
FROM EMP;
...
SELECT CONCAT( LOWER( ENAME ), INITCAP( JOB ) ) AS "员工信息"
FROM EMP;
...
SELECT E.ENAME, E.SAL, E.COMM, ( E.SAL * 12 + NVL( E.COMM, 0 )) AS 年收入
FROM EMP E;
