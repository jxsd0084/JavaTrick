 ...
 * @date 2013-7-19
 */
public class AccessLogScanner extends BaseScanner implements Runnable {

	private static final Logger    logger    = Logger
			.getLogger( AccessLogScanner.class );
	...
	// 是否只查询当前也数据，数据导出时设置为false
	private              boolean limited  = true;
	//是否导出
	private static final String  ISEXPORT = "ISEXPORT";

	public AccessLogScanner( Session session, TCommonCondition tcc,
	...
	public void run() {
		...
		//如果导出  直接返回
		if ( condition.getAttributes() != null && condition.getAttributes().get( ISEXPORT ).equals( "1" ) ) {
			session.setErrorcode( 999 );
			session.setErrormsg( "数据导出中" );
			session.notifyMe();
			ExecutorService exec = Executors.newCachedThreadPool();
			exec.execute( new AccessLogExportScanner( session, condition, baseTable, coprocessor, conf ) );
			exec.shutdown();
			return;
		}
		// 检查参数是否合法
...
//					System.out.println(new String(r.getValue("f".getBytes(), "c".getBytes())));
//					System.out.println(new String(r.getValue("f".getBytes(), "d".getBytes())));
//					System.out.println(new String(r.getValue("f".getBytes(), "e".getBytes())));
//					System.out.println(new String(r.getValue("f".getBytes(), "f".getBytes())));
//					System.out.println("--------------------------------");
//				}			
				...
				if ( nr != null && nr.getResult().size() > 0 ) {
					...
					rssize = rssize + nr.getResult().size();
					// 当数据足够后，不再查询后面的表
		...
		page = buildRestut( tableInfo, page, qr );
		session.notifyMe();
