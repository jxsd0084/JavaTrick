...
public class FilterLogScanner extends BaseScanner implements Runnable {

	Logger logger = Logger.getLogger( FilterLogScanner.class );
	private static final TableInfo tableInfo  = IdcTableInfos
			.getTableInfo( TableConstants.TO_NET_ODS_FILTERLOGS );
	// KEY 长度
	private static final int       KEY_LENGTH = TableConstants.TABLE_FILTERLOGS_KEY_LENGTH;
	...
	public FilterLogScanner( Session session, TCommonCondition tcc,
	...
	public void run() {
		...
		for ( String table : tables ) {
			...
			try {
				...
				// System.out.println(new String(r.getValue(Bytes.toBytes("f"),
				// Bytes.toBytes("c"))));
				// System.out.println(new String(r.getValue(Bytes.toBytes("f"),
				// Bytes.toBytes("d"))));
				// System.out.println(new String(r.getValue(Bytes.toBytes("f"),
