...
public class MonitorLogScanner extends BaseScanner implements Runnable {

	Logger logger = Logger.getLogger( MonitorLogScanner.class );
	private static final TableInfo tableInfo  = IdcTableInfos.getTableInfo( TableConstants.TO_NET_ODS_MONITORLOGS );
	//key length
	private static final int       KEY_LENGTH = TableConstants.TABLE_MONITORLOGS_KEY_LENGTH;
	...
	//在结果中包含 时间字段
	private String  RESULT_GATHER_TIME = "GATHER_TIME";
	
	public MonitorLogScanner( Session session, TCommonCondition tcc,
...
//				}
	...
	protected List< String > getMetadata( TableInfo tableInfo ) {
		...
		metadata.add( RESULT_GATHER_TIME );
	...
	/**
	 * 检查客户端输入条件是否合法，检查时间格式是否正确（yyyy-MM-dd HH:mm:ss）
	 *
	 * @param condition
	...
	private void checkCondition( MainCondition condition )
		...
		try {
			...
			//start = formatDate("2014-02-21 20:00:00");
		...
		try {
			...
			//end = formatDate("2014-02-21 20:59:59");
		} catch ( ParseException e ) {
			throw new ClientParameterException(
	...
	private Scan createScan() {
		...
		byte[] endKey = ByteUtils
				.createArray( KEY_LENGTH, ( byte ) Byte.MAX_VALUE );
		...
		byte[] bend = ConvertUtils.convertDateToFullString( end )
				.getBytes();
		System.arraycopy( bstart, 0, startKey, 6, 14 );
		System.arraycopy( bend, 0, endKey, 6, 14 );
			...
			for ( Map.Entry< String, Filter > set : filters.entrySet() ) {
				...
				if ( field.equals( REQUIRED_HOUSE_ID ) ) {
					...
					valuefilter.append( tableInfo.getValueFilterStr(
							REQUIRED_HOUSE_ID, OperatorType.EQ,
							filters.get( REQUIRED_HOUSE_ID ).getValue() ) );
					// valueFilter中添加了一条数据后 first置为 false
					first = false;
					continue;
				}
				OperatorType type = getOperatorType( filter.getMatchType()
						.getValue() );
				valuefilter.append( tableInfo.getValueFilterStr( field, type,
						filter.getValue() ) );
				first = false;
			}
